...
public class LoggingAspect {
     ...
     *   => com.ohgiraffers.section01.advice 패키지에 속해 있는 바로 다음 하위 클래스에 파라미터가 1개인
     *      모든 메소드이자 접근 제어자가 public이고 반환형이 Integer인 경우
     *   execution(* com.ohgiraffers.section01.advice.annotation..stu*(..))
     *   => com.ohgiraffers.section01.advice 패키지 및 하위 패키지에 속해 있는 모든 클래스 중에서
     *      이름이 stu로 시작하는 파라미터가 0개 이상인 모든 메소드이며 접근제어자와 반환형은 상관없음
    * */
...
//        System.out.println("After Advice 동작");
    ...
    /* 설명. 5. Around Advice */
    /* 설명.
     *  Around Advice는 JoinPoint를 완전히 장악하기 때문에 앞서 살펴 본 Advice 모두 Around Advice로 조립할 수 있다
     *  심지어 원본 JoinPoint를 언제 실행할 지, 실행 자체를 안할 지, 계속 실행할 지 여부까지도 제어한다
     *  Around Advice의 JoinPoint 매개변수는 ProceedJoinPoint로 고정되어 있다
     *  JoinPoint의 하위 인터페이스로 원본 JoinPoint의 진행 시점을 제어할 수 있다
     *  JoinPoint를 진행하는 호출을 잊는 경우가 자주 발생하기 때문에 주의해야하며
     *  가능한 최소한의 요건을 충족하면서도 가장 기능이 약한 Advice를 쓰는게 바람직하다.
    * */
    @Around("logPointcut()")
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        /* 설명. 실행된 타겟 메소드 반환(다른 Advice가 다시 활용할 수 있도록 반환한다) */
        return result;
    }
